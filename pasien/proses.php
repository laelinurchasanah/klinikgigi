...
if(isset($_POST['add'])){
	...
	$tanggallahir = trim(mysqli_real_escape_string($con, $_POST['tanggallahir']));
	...
	if(mysqli_num_rows($sql_cek_identitas) > 0){
		...
		mysqli_query($con, "INSERT INTO tb_pasien VALUES('$uuid', '$identitas', '$nama','$tanggallahir', '$jk', '$alamat', '$telp')") or die(mysqli_error($con));
	...
	if(mysqli_num_rows($sql_cek_identitas) > 0){
		...
		mysqli_query($con, "UPDATE tb_pasien SET nomor_identitas='$identitas', nama_pasien='$nama', tanggallahir='tanggallahir', jenis_kelamin='$jk', alamat='$alamat', no_telp='$telp' WHERE id_pasien='$id'") or die(mysqli_error($con));
		echo "<script>alert('Data berhasil ditambahkan');window.location='data.php';</script>";
	}
}
	...
	$all_data = $obj->getActiveSheet()->toArray(null, true, true, true,true);
	...
	for ($i=3; $i <=count($all_data)  ; $i++){ 
		...
		$no_id = $all_data[$i]['A'];
		$nama = $all_data[$i]['B'];
		$tanggallahir = $all_data[$i]['C'];
		$jk = $all_data[$i]['D'];
		$alamat = $all_data[$i]['E'];
		$telp = $all_data[$i]['F'];
		$sql .= "('$uuid', '$no_id', '$nama','$tanggallahir', '$jk', '$alamat', '$telp'),";
	}
	$sql = substr($sql, 0, -1);
	// echo $sql;
