	...
	<div class="box">
		...
		<h4>
			...
			<div class="pull-right">
				...
				<a href="add.php" class="btn btn-success btn-xs"><i class="glyphicon glyphicon-plus"></i> Tambah Resep</a>
			</div>
		</h4>
		<form method="post" name="proses">
			<div class="table-responsive">
				<table class="table table-bordered table-striped table-hover" id="dokter">
					...
					<?php
						$no = 1;
						$sql_tindakan = mysqli_query($con, "SELECT * FROM tb_resep") or die(mysqli_error($con));
						while($data = mysqli_fetch_array($sql_tindakan)){ ?>
							<tr>
								<td align="center">
									<input type="checkbox" name="checked[]" class="check" value="<?= $data['id_resep']; ?>">
								</td>
								<td><?= $no++; ?>.</td>
