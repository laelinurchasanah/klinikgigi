			...
			<div class="col-lg-8 col-lg-offset-2">
				<form action="proses.php" method="post">
					...
					<table class="table">
						...
						$no = 1;
						foreach($chk as $id){
							$sql_tindakan = mysqli_query($con, "SELECT * FROM tb_tindakan WHERE id_tindakan='$id'") or die(mysqli_error($con));
							while($data = mysqli_fetch_array($sql_tindakan)){ ?>
								<tr>
									<td><?= $no++; ?></td>
									<td>
										<input type="hidden" name="id[]" value="<?= $data['id_tindakan']; ?>">
										<input type="text" name="nama[]" value="<?= $data['nama_tindakan'] ?>" class="form-control" required>
									</td>
									<td>
