...
if(isset($_POST['add'])){
	...
	for($i=1; $i<=$total; $i++){
		...
		$sql = mysqli_query($con, "INSERT INTO tb_tindakan VALUES('$uuid', '$nama', '$harga')") or die(mysqli_error($con));
	}
	...
	for($i=0; $i<count($_POST['id']); $i++){
		...
		$nama = $_POST['nama'][$i];
		$harga = $_POST['harga'][$i];
		$sql = mysqli_query($con, "UPDATE tb_tindakan SET nama_tindakan='$nama', harga='$harga' WHERE id_tindakan='$id'") or die(mysqli_error($con));
	}
